    ...
    <div style="padding:10px 20px;">
        ...
        <div style="display: flex;margin: 20px 0;">
            ...
            <div class="col-lg-6" style="margin-right: -15px;">
                ...
                <div style="background-color: #F5F5FA;margin: 10px 0; padding: 10px; border-radius: 2px;min-height: 100px;">
        ...
        <div style="width: 100%;">
            @foreach($OrderDetail->OrderItem as $OrderDetailDetail)
            <div style="margin: 20px; display: flex; align-items: center;">
                <div
                    ...
                    <img style="width: 100%" src='{{$OrderDetailDetail->product->product_thumb}}' />
                </div>
                <div style="display: flex; flex-direction: column; margin-left: 20px;">
                    <p class="long-text" style="font-size: 16px;max-width: 700px !important">
                        {{$OrderDetailDetail->product->product_name}}</p>
                    <span style="margin: 0; font-size: 12px; text-transform: uppercase;">Size:
                        {{$OrderDetailDetail->od_item_size}}</p>
                        <span style=" font-size: 12px">
                            {{number_format($OrderDetailDetail->od_item_price, 0, ',', '.')}}₫ X <span
                                style="color: #FE6133;">{{$OrderDetailDetail->od_item_quantity}}</span></p>
                </div>
            </div>
            @endforeach
